...
import {
  div, p, span, input as inputEl, img,
} from '../../scripts/dom-helpers.js';
import {
  fetchBookARide, fetchOTP, useDataMapping, verifyOtp, getRandomId, debounce,
...
function createDropdownInput(placeholder, name) {
  const input = inputEl({
    placeholder, class: 'react-select__input', autocomplete: 'off', name,
  });
...
export default async function decorate(block) {
  ...
  const stateInp = form.state;
  const stateFieldWrapper = stateInp.closest('.field-wrapper');
  const cityInp = form.city;
  const cityFieldWrapper = cityInp.closest('.field-wrapper');

  const {
    wrapper: stateCustomWrapper,
    input: stateCustomInput,
    clearBtn: stateClearBtn,
    dropdownBtn: stateDropdownBtn,
    list: stateList,
  } = createDropdownInput('Select State', 'state');
  const {
    wrapper: cityCustomWrapper,
    input: cityCustomInput,
    clearBtn: cityClearBtn,
    dropdownBtn: cityDropdownBtn,
    list: cityList,
  } = createDropdownInput('Select City', 'city');

  stateCustomInput.id = 'state-input';
  cityCustomInput.id = 'city-input';

  stateInp.replaceWith(stateCustomWrapper);
  stateFieldWrapper.appendChild(stateList);

  cityInp.replaceWith(cityCustomWrapper);
  cityFieldWrapper.appendChild(cityList);
  ...
  if (initialCurrentLocation.state) {
    selectedStateData = mappedStates
      .find((s) => s.label.toUpperCase() === initialCurrentLocation.state.toUpperCase());
    if (selectedStateData) {
      stateCustomInput.value = selectedStateData.label;
      if (initialCurrentLocation.city) {
        selectedCityData = selectedStateData.cities
          .find((c) => c.code.toUpperCase() === initialCurrentLocation.city.toUpperCase());
        if (selectedCityData) {
  ...
  stateClearBtn.addEventListener('click', () => {
    ...
    showError(stateFieldWrapper, 'Select a state');
    showError(cityFieldWrapper, 'Select a city');
  ...
  function checkValidity() {
    ...
    return [...allInp].map((inp) => {
      ...
      if (inpName === 'state') {
        return validateOtherFields(stateFieldWrapper, inpName, stateCustomInput.value);
      } if (inpName === 'city') {
        return validateOtherFields(cityFieldWrapper, inpName, cityCustomInput.value);
      } if (inpName === 'name') {
