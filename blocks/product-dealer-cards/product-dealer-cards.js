...
async function decorateProductDealerCards(block = document.querySelector('.product-dealer-cards')) {
  ...
  let activeState = (current.state && cityMap[current.state.toUpperCase()])
    ? current.state
    : states[0];
  ...
  async function renderDealers(stateLabel, cityCode) {
    ...
    const swiperEl = block.querySelector('.dealer-card-wrapperr');
    ...
    const cityList = Object.values(cityMap[stateLabel.toUpperCase()] || []);
    const cityData = cityList.find((c) => c.code === cityCode);
    if (!cityData) {
    ...
    swiperEl.swiper = new Swiper(swiperEl, {
      ...
      observeParents: true,
      breakpoints: {
        0: { slidesPerView: 1.56, spaceBetween: 16 },
        768: { slidesPerView: 2 },
  ...
  const onStateSelect = async (newState) => {
  ...
  const onCitySelect = async (newCity) => {
    activeCity = newCity;
  ...
  const initialCityList = Object.values(
  ...
  if (current.state && cityMap[current.state.toUpperCase()]) {
    ...
    const initialCityObj = Object.values(
      cityMap[activeState.toUpperCase()] || {},
    ).find(
