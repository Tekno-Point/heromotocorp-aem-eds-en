...
const rotateImg = (event, imgEl, rotateUrlString, isTouch = false) => {
  isDragging = true;
...
export async function decorateProductBanner(block) {
  let heading;
  let bottomSection;
  ...
  const updateActiveColorSwatch = (colorLabel) => {
    block.querySelectorAll('.color-option').forEach((option) => option.classList.toggle('active', option.querySelector('span').textContent === colorLabel));
  };
    ...
    colors.forEach(({ sku, label: colorLabel, color_swatch_url: colorSwatchUrl }) => {
      const option = div(
        ...
        img({
          class: 'swatch-color',
          loading: 'lazy',
          src: `https://www.heromotocorp.com${colorSwatchUrl}`,
          alt: colorLabel,
        }),
      );
  ...
  const variantsDOM = div(
    { class: 'product-banner__variantWrapper' },
    div(
      { class: 'variants-wrap' },
      ...
      div(
        ...
        ...variantsData.map(({
          ...
          return div(
            { class: `product-form-control  ${isActive ? 'active' : ''}` },
            div(
  ...
  mainImage.addEventListener('mousedown', (e) => {
    ...
    rotateImg(e, mainImage, media.product.media_gallery);
  });
  mainImage.addEventListener('touchstart', (e) => {
    const media = getVariantDetailsBySku(dataMapping.sku);
    rotateImg(e.touches[0], mainImage, media.product.media_gallery, true);
  });
